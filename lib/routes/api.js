...
function socketResponse(socket) {
	...
	socket.on('join-room', val => {
		...
		if (typeof socket.tokenData === 'object' && database.isRoom(val.room)) {
	...
	socket.on('add-message', val => {
		...
		if (val.room && val.nickname && tokenData && val.message) {
			socket.to(socket.room).emit('message', {text: val.text, from: val.nickname});

			if (socket.tokenData.user !== 'none') {
				database.recordChat(val.room, tokenData.email, val.text);
			} else {
				database.recordChat(val.room, val.nickname, val.text);
			}

			socket.emit('message', {text: val.text, from: val.nickname});
		}
