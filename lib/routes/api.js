...
router.get('/get-chats', (req, res) => {
	...
	if (database.isRoom(room)) {
		res.send( responses.success(database.getChats(room)) );
...
function socketResponse(socket) {
	socket.on('chat message', arr => {
		let [room, msg, token] = JSON.parse(arr);
		token = tokenGen.verifyAndDecrypt(token);

		if (room && msg && token) {
			database.addChat(room, token.email, msg);
			socket.join(room);
